...
def _package_evidence(candidate_pt_ids, event_df, trend_df, is_subset_query) -> dict:
    ...
    evidence_package = {
        "retrieved_candidates": len(candidate_pt_ids),
        "query_scope": "subset" if is_subset_query else "cohort",
        "subsampled": False,
        "cta_allowed": (not is_subset_query) and len(candidate_pt_ids) > 4,
    }
    event_df = event_df.copy(deep=True) if event_df is not None else pd.DataFrame(columns=["patient_id"])
    no_unit_str = "_none_"
    if not event_df.empty:
        event_df["unit"] = event_df["unit"].fillna(no_unit_str)  # necessary because None's throw groupby off
    trend_df = trend_df.copy(deep=True) if trend_df is not None else pd.DataFrame(columns=["patient_id"])
    event_df_qe = event_df.loc[~event_df["context"]] if "context" in event_df else event_df
    event_df_ctx = event_df.loc[event_df["context"]] if "context" in event_df else event_df
    patients_map = {}
    ...
    if len(event_df_qe) + len(trend_df) <= settings.params.max_qe_datapoints:  # enough space for all entries
        ...
        0
        if event_df_qe.empty
        else event_df_qe.groupby(
            ["patient_id", "normalized_term", "unit", "is_active", "is_started", "is_ended"],
            sort=False,
        ).ngroups
    ) + len(
        trend_df
    ) <= settings.params.max_qe_datapoints:  # enough space when events are summarized
        if not event_df_qe.empty:
            for (pid, pname, page, psex, _, _, _, _, _), df_pt in event_df_qe.groupby(
                [
                    "patient_id",
                    "patient_name",
                    "age",
        ...
        items_per_pt = (
            ...
            / event_df_qe["patient_id"].nunique()
            if not event_df_qe.empty
            else 0
        ) + (len(trend_df) / trend_df["patient_id"].nunique() if not trend_df.empty else 0)
        candidate_pt_ids = random.sample(
            candidate_pt_ids,
            min(int(0.9 * settings.params.max_qe_datapoints / items_per_pt), len(candidate_pt_ids)),
        )
    ...
    if len(event_df_ctx) <= settings.params.max_context_datapoints:  # enough space for all context entries
        ...
        event_df_ctx.groupby(
            ["patient_id", "normalized_term", "unit", "is_active", "is_started", "is_ended"],
            sort=False,
        ).ngroups
        <= settings.params.max_context_datapoints
    ):  # enough space for all context when summarized
        if not event_df_ctx.empty:
            for (pid, pname, page, psex, _, _, _, _, _), df_pt in event_df_ctx.groupby(
                [
                    "patient_id",
                    "patient_name",
                    "age",
    ...
    # check if there is enough space for summarized entries from the context, only considering the terms that appear in the qualifying evidence
        ...
        event_df_ctx[event_df_ctx["normalized_term"].isin(qualifying_terms)]
        .groupby(
            ["patient_id", "normalized_term", "unit", "is_active", "is_started", "is_ended"],
            sort=False,
        )
...
def _create_event_term_entry(r, no_unit_str) -> dict:
    ...
    if not pd.isna(ts_start):
        ts_start = int(ts_start)
    ts_end = getattr(r, "day_offset_end", ts_start)
    if not pd.isna(ts_end):
        ts_end = int(ts_end)
    item = {
        "type": "event",
...
def _create_trend_term_entry(r, no_unit_str) -> dict:
    ...
    if not pd.isna(ts1_start):
        ts1_start = int(ts1_start)
    ts1_end = getattr(r, "anchor1_end", ts1_start)
    if not pd.isna(ts1_end):
        ts1_end = int(ts1_end)
    ts2_start = getattr(r, "anchor2_start", None)
    if not pd.isna(ts2_start):
        ts2_start = int(ts2_start)
    ts2_end = getattr(r, "anchor2_end", ts2_start)
    if not pd.isna(ts2_end):
        ts2_end = int(ts2_end)
    item = {
        "type": "trend",
        "term": getattr(r, "normalized_term", None),
...
def _create_event_summary_entry(g: pd.DataFrame, no_unit_str: str) -> dict:
    ...
    if n_numeric > 0:
        ...
        if n_numeric > 1:
            item["stats"]["min_entry_timestamp"] = int(ts.loc[vals_num.idxmin()]) if pd.notna(ts.loc[vals_num.idxmin()]) else None
        if n_numeric > 1:
            item["stats"]["max_entry_timestamp"] = int(ts.loc[vals_num.idxmax()]) if pd.notna(ts.loc[vals_num.idxmax()]) else None
...
def _generate_final_answer(client, user_query, packaged_evidence_dict) -> str:
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": "Patient evidence JSON:\n```json\n" + user_payload_json + "\n```",
        },
        {"role": "user", "content": final_llm_call_rubric()},
    ]

    resp = client.chat.completions.create(
        model=settings.params.gpt_model,
        temperature=settings.params.final_response_temperature,
        messages=messages,
    )
