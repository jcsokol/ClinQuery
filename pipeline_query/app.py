...
st.markdown(
    """<div style="font-size: 2.5rem;font-weight: 600;line-height: 1.2;margin: 0 0 1.00rem 0;">EHR Query Engine Demo</div>""",
    unsafe_allow_html=True,
)
st.markdown(
    """
...
def load_compiler() -> QueryCompiler:
    ...
    return QueryCompiler(
        duckdb_path=settings.params.duckdb_path,
        faiss_path=settings.params.faiss_path,
        idmap_path=settings.params.idmap_path,
        embed_fn=embed_fn,
    )
...
if st.session_state.last_result:
    ...
    with st.expander("Show reasoning trace", expanded=False):
        ...
        st.markdown(
            "**Status:** "
            + str(result.get("run_status"))
            + "  ·  "
            + "**Retrieved candidates:** "
            + str(result.get("n_candidates"))
            + "  ·  "
            + "**Latency:** "
            + str(round(result.get("total_latency"), 2))
            + "s"
        )
        ...
        with st.expander("SQL query", expanded=False):
            ...
            if int(result.get("n_candidates")) > 0:  # skip if no candidates were retrieved
                ...
                if event_df is not None:
                    ...
                    st.caption("Row count: " + str(len(event_df)))
                if trend_df is not None:
                    if event_df is not None:
                        st.markdown("**Retrieved precomputed trends**")
                    st.dataframe(trend_df, use_container_width=True, hide_index=True)
                    st.caption("Row count: " + str(len(trend_df)))
