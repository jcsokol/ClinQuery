...
class QueryCompiler:
    ...
    def resolve(self, q_raw: str) -> tuple[
        ResolvedTarget | None,
        ResolvedTarget | None,
        ResolvedTarget | None,
        dict[str, list[str]],
        ...
        # comparator for "best"
        def better(a: ResolvedTarget | None, b: ResolvedTarget) -> bool:
            if a is None:
                return True
            if b.score != a.score:
        ...
        best_term: ResolvedTarget | None = None
        best_class: ResolvedTarget | None = None
        best_custom: ResolvedTarget | None = None
        ...
        for alias_str, score, _ in hits:
            ...
            for aid in self.alias_to_ids[alias_str]:
                ...
                if kind == "term":
                    if better(best_term, rt):
                        best_term = rt
                elif kind == "concept_class":
                    if better(best_class, rt):
                        best_class = rt
        ...
        if (
            ...
            D, idx = self.faiss.search(q2, settings.params.ann_topk)

            seen = set()
            for d, row in zip(D[0], idx[0], strict=False):
                if row < 0:
                    continue
    ...
    def load_custom_concepts(self, dir_path: str) -> tuple[dict[str, dict[str, Any]], dict[str, set[str]]]:
        ...
        for fp in files:
            ...
            custom_concepts[concept_key] = {
                "aliases": aliases,
                "definition_polarity": pol,
    ...
    def normalize_yaml_definition_to_intent_tree(
        self,
        custom_key: str,
        yaml_node: dict,
        lookback_window: dict,
        temporal: dict | None,
        unresolved_set: set,
    ):
    ...
    def inject_custom_constraints_into_trace(self, trace) -> dict[str, Any]:
        ...
        for _cl_i, cl in enumerate(clauses):
            ...
            normalized_root, unresolved_custom = self.normalize_yaml_definition_to_intent_tree(
                custom_key,
                cc["definition"],
                cc["lookback_window"],
                tree_temporal,
