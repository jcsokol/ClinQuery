...
def chunk_text(example, tokenizer, max_length=512, overlap=0.25):
    ...
    for start in range(0, len(input_ids), stride):
        ...
        chunks.append({"text": chunk_text, "spans": chunk_spans})
...
def convert_spans_to_bio_labels(example, tokenizer, label2id, max_length=512, expand_entities=False):
    ...
    # Define priority
    PRIORITY = {"PLAN": 3, "TIME": 2, "C_ENT": 1}
    ...
    encoding["labels"] = [label2id.get(l, 0) if o != (0, 0) else -100 for l, o in zip(labels, offsets)]
    ...
    with input_path.open("r", encoding="utf-8") as infile:
        for line in infile:
            ...
            for rel in relations:
                ...
                child_id = rel["child"]
                label = rel["label"]
                example_relations.append({"head": head_id, "child": child_id, "label": label})

            output.append(
                (
                    raw_text,
                    text.replace("condition_symptom_measurement_device_procedure_med", "c_ent"),
                    example_relations,
                )
            )
