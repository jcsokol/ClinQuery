...
def _validate_json_str(json_str: str) -> tuple[bool, str | None]:
...
def _semantic_validate(intent: dict[str, Any]) -> str | None:
    """
    ...
    for i, cl in enumerate(clauses):
        ...
        #     elif (_has_from and _has_until and "baseline" in _mt_lower):
...
def call_intent_parser_once(client: OpenAI, query: str, model: str = "gpt-4.1", temperature: float = 0) -> str:
    ...
    chat = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Call emit_parse with the final JSON as arguments. Do not write any text.",
            },
...
def retry_fix_json(
    ...
    msgs = [
        {
            "role": "system",
            "content": "Fix the output by calling emit_parse. Do not write any text; only the function call.",
        },
        {"role": "user", "content": _intent_parser_prompt()},
        {"role": "user", "content": query},
    ...
    if bad_snippet:
        msgs.append(
            {
                "role": "user",
                "content": f"Here is your previous output to correct:\n```json\n{bad_snippet}\n```",
            }
        )

    last_err = None
    for _attempt in range(settings.params.intent_parser_max_retries + 1):
        try:
            chat = client.chat.completions.create(
                model=model,
                temperature=temperature,
...
def parse_with_repair(client: OpenAI, query: str, model: str = "gpt-4.1", temperature: float = 0) -> dict[str, Any]:
    ...
    if ok_schema:
        ...
        # Semantic repair
        time.sleep(1.0)
        repaired = retry_fix_json(
            client,
            query,
            bad_json=first,
            schema_error=f"Semantic error: {sem_err}",
            model=model,
            temperature=temperature,
        )
    ...
    # Schema repair path
    time.sleep(1.0)
    repaired = retry_fix_json(client, query, bad_json=first, schema_error=err_schema, model=model, temperature=temperature)
    ok2, err2 = _validate_json_str(repaired)
    if ok2:
        repaired_json = json.loads(repaired)
        sem_err2 = _semantic_validate(repaired_json)
        ...
        time.sleep(1.0)
        repaired2 = retry_fix_json(
            client,
            query,
            bad_json=repaired,
            schema_error=f"Semantic error: {sem_err2}",
            model=model,
            temperature=temperature,
        )
