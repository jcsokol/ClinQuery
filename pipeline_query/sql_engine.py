...
class SqlEngine:
    ...
    def __init__(self, duckdb_path: str, pump_canonical):
        ...
        assert self.pump_canonical is not None  # ensure that its canonical could be resolved
    ...
    def run_evidence_sql(
        self,
        patient_ids: list[str],
        sql_event: str,
        params_event: list[int | float | str | bool],
        sql_trend: str,
        params_trend: list[int | float | str | bool],
    ...
    def _compile_leaf(self, leaf, temporal_ctx):
        ...
        if mod == "absent":
            ...
            if mode == "history":
                if term:
                    s, p = self._term_history_absent_predicate(term)
                    disj_sql.append(s)
                    disj_params += p
                if concept_class:
                    s, p = self._class_history_absent_predicate(concept_class)
                    disj_sql.append(s)
                    disj_params += p
            ...
            if mode == "baseline":
                if term:
                    s, p = self._term_baseline_absent_predicate(term)
                    disj_sql.append(s)
                    disj_params += p
                if concept_class:
                    s, p = self._class_baseline_absent_predicate(concept_class)
                    disj_sql.append(s)
                    disj_params += p
                return (" AND ".join(f"({s})" for s in disj_sql) or "TRUE", disj_params)
            ...
            if term:
                s, p = self._term_absent_predicate(term, spec)
                disj_sql.append(s)
                disj_params += p
            if concept_class:
                s, p = self._class_absent_predicate(concept_class, spec)
                disj_sql.append(s)
                disj_params += p
            return (" AND ".join(f"({s})" for s in disj_sql) or "TRUE", disj_params)
        ...
        if mode == "history":
            if term:
                s, p = self._term_history_predicate(leaf, term)
                disj_sql.append(s)
                disj_params += p
            if concept_class:
                s, p = self._class_history_predicate(leaf, concept_class)
                disj_sql.append(s)
                disj_params += p
        ...
        if mode == "baseline":
            if term:
                s, p = self._term_baseline_predicate(leaf, term)
                disj_sql.append(s)
                disj_params += p
            if concept_class:
                s, p = self._class_baseline_predicate(leaf, concept_class)
                disj_sql.append(s)
                disj_params += p
            return (" OR ".join(f"({s})" for s in disj_sql) or "TRUE", disj_params)
        ...
        spec = self._resolve_temporal_day_window(temporal_ctx)
        if term:
            s, p = self._term_exists_predicate(leaf, term, spec)
            disj_sql.append(s)
            disj_params += p
        if concept_class:
            s, p = self._class_exists_predicate(leaf, concept_class, spec)
            disj_sql.append(s)
            disj_params += p
        return (" OR ".join(f"({s})" for s in disj_sql) or "TRUE", disj_params)
            ...
            if rel == "UNTIL":
                if a.get("form") == "admission":
                    return WindowSpec(
                        "absolute",
                        -INF,
                        int(a.get("day_offset") or 0),
                        None,
                        None,
                        None,
                        [],
                        include_history=(int(a.get("day_offset") or 0) <= 0),
                    )  # last entry set to true for the special case: UNTIL admission day 0
    ...
    def _term_exists_predicate(self, leaf, term, spec: WindowSpec):
        ...
        conds.append("e.normalized_term = ?")
        params.append(term)
        conds.append("e.is_active = TRUE")
        if leaf.get("op") and (leaf.get("value") is not None):
            conds.append(f"e.value {leaf['op']} ?")
            params.append(leaf["value"])
        if leaf.get("unit"):
            conds.append("e.unit ILIKE '%' || ? || '%'")
            params.append(str(leaf["unit"]))
        ...
        if spec.include_history:
            ...
            h_conds.append("e.normalized_term = ?")
            h_params.append(term)
            h_conds.append("e.is_active = TRUE")
            h_conds.append("e.history = TRUE")
            if leaf.get("op") and (leaf.get("value") is not None):
                h_conds.append(f"e.value {leaf['op']} ?")
                h_params.append(leaf["value"])
            if leaf.get("unit"):
    ...
    def _class_exists_predicate(self, leaf, concept_class, spec: WindowSpec):
        ...
        """
        # main (window-overlap) branch
        base_join = "FROM event_df e " "JOIN term_concept_class_map_df m " "ON m.term = e.normalized_term AND m.concept_class = ?"
    ...
    def _class_history_predicate(self, leaf, concept_class):
        ...
        join = "FROM event_df e " "JOIN term_concept_class_map_df m " "ON m.term = e.normalized_term AND m.concept_class = ?"
    ...
    def _term_baseline_predicate(self, leaf, term):
        ...
        if has_numeric:
            ...
            conds.append("b.normalized_term = ?")
            params.append(term)
            conds.append("b.anchor = 'admission_start'")
            conds.append(f"b.value {leaf['op']} ?")
            params.append(leaf["value"])
            if leaf.get("unit"):
                conds.append("b.unit ILIKE '%' || ? || '%'")
                params.append(str(leaf["unit"]))
    ...
    def _term_absent_predicate(self, term, spec: WindowSpec):
        ...
        neg_conds.append(ov_sql)
        neg_params += ov_params
        join_sql = f" {spec.join} " if (spec.mode == "per_patient" and spec.join) else " "
    ...
    def _resolve_trend_anchors(self, temporal_ctx) -> tuple[str, str] | None:
        ...
        def pick_anchors(tctx):
            ...
            if len(tctx) == 1:
                ...
                if (a_term0 == self.pump_canonical.canonical and (a_day_offset0 is None or a_day_offset0 == 0)) and (
                    a_term1 == self.pump_canonical.canonical and (a_day_offset1 is None or a_day_offset1 == 0)
                ):
    ...
    def _emit_event_leaf_selects(
        ...
        if leaf.get("op") and (leaf.get("value") is not None):
            num_where.append(f"e.value {leaf['op']} ?")
            num_params.append(leaf["value"])
        if leaf.get("unit"):
            num_where.append("e.unit ILIKE '%' || ? || '%'")
            num_params.append(str(leaf["unit"]))
    ...
    def _compile_event_evidence_sql(self, trace: dict, patient_ids: list[str]) -> tuple[str | None, list | None]:
        ...
        for t in anchor_terms:
            sql = "SELECT e.* FROM event_df e " "WHERE " + pid_clause + " AND e.normalized_term = ?"
    ...
    def _compile_candidate_sql(self, trace):
        ...
        candidates_cte = "candidates AS (" "  SELECT p.patient_id FROM patient_df p " f"  WHERE {pr_sql}" ")"
        if ctes:
