...
def run_query(user_query: str, session_id: str, query_compiler: Callable):
    ...
    try:  # try running entire query
        ...
        payload_dict["intent_generation_latency"] = time.perf_counter() - timer_start
        ...
        payload_dict["abstraction_layer_latency"] = time.perf_counter() - timer_start_int
        timer_start_int = time.perf_counter()
        ...
        payload_dict["sql_query_latency"] = time.perf_counter() - timer_start_int
        timer_start_int = time.perf_counter()
        # Evidence packaging & final llm call
        is_subset_query = len(payload_dict["parsed_intent"]["patient_references"]) > 0
        payload_dict["final_answer"], payload_dict["final_llm_call_input"] = generate_response(
            user_query,
            candidate_pt_ids,
            payload_dict["sql"]["retrieved_event_df"],
            payload_dict["sql"]["retrieved_trend_df"],
            is_subset_query,
        )
        payload_dict["final_llm_call_latency"] = time.perf_counter() - timer_start_int
        timer_start_int = time.perf_counter()
